...
**導入新專案**：複製 `CLAUDE.md` + `.eslintrc.json` + `tsconfig.json`，設置 `package.json` 版本釘選和 `wrangler.toml`。

**保持不變**：CLAUDE.md / ESLint 規則 / runtime.ts。
**可自定義**：DB schema / pages / components / config。

**專案 CLAUDE.md 引用方式**：
```
## Cloudflare Stack Framework
核心規則見：/path/to/cloudflare/CLAUDE.md
以下為專案特定擴展規則...
```
...
| 可擴展 | 不可擴展 |
|---|---|
| 業務邏輯規則 / UI 組件約定 | 核心技術棧 / 基礎架構原則 |
| 專案特定測試 / 環境變數 | 框架測試基礎 / Runtime Gateway |
