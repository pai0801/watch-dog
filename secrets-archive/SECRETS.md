...
| `CF_USAGE_API_TOKEN` | `GET /api/cf-usage` 對外唯讀用量 API 的靜態 Bearer token（`timingSafeEqual` 常數時間比對、fail-closed；缺值一律 401） | `openssl rand -hex 32`（操作者本機產生） | **值存放**：域 A＝`~/.config/watch-dog/usage-api-token`（操作者本機明文檔，chmod 600，repo 外——其它專案經此檔取得）＋ `.dev.vars`（seal 進 `env.7z`）；域 B＝Worker secret（`wrangler secret put CF_USAGE_API_TOKEN`，file-sourced pipe，agent 不經手明文）。**被誰用**：其它專案的 Claude Code／自動化（README「CF 用量 API」記載用法與本機檔路徑） | **輪替**：重新 `openssl rand -hex 32` → 覆寫本機檔 → `wrangler secret put CF_USAGE_API_TOKEN < 檔` → `.dev.vars` 同步＋`seal.sh` reseal → 通知消費端專案 | **2026-09-12（首次設定）** |
...
| `cf_accounts.api_token` ×8（**非 Worker secret**——D1 `cf_accounts` 表，2026-09-11 起） | CF 全帳號用量監控：每 30 分鐘經 GraphQL Analytics API（`/client/v4/graphql`）輪詢各帳號 D1/Workers/KV/R2 用量，60%/80% 閾值＋速率投影警示（9/7、9/11 兩次 D1 額度事故的主動預警層） | 操作者於各帳號 dash.cloudflare.com → My Profile → API Tokens mint（**Custom Token、僅 Account→Analytics: Read、單帳號 scope**，名稱 `watch-dog-usage-<label>`；值只顯示一次，存密碼管理器） | D1 `cf_accounts` 表（`/admin` → CF 用量 tab 表單管理，遮罩顯示、空欄=保留；`src/services/cfUsage.ts` 輪詢時讀取） | 單帳號 token 輪替：/admin → 該帳號 Edit → 貼新值（舊值即時失效，下輪 poll 生效）；誤設過大權限 [MUST] 到 dashboard 作廢重 mint。**值 [NEVER] 進 repo/chat/agent**（操作者經表單自貼） | 2026-09-11 功能上線；8 帳號 = 8fdbf0ee（已驗證）、c8be0aed（已驗證）、79261d85、db225a4d、cb34e1ba、272a2d60、30f134f1、7354e81a（後 6 支操作者待 mint） |
