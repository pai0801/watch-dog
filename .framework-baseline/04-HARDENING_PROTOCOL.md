...
> D18 驗「rule 有登記」、D34 驗「guard 存在·唯一·接線」,都沒驗「guard 真的有做事」。
> **D38 補這層**:每個 guard test [MUST] 自帶兩個 marker(語言中立 comment):
> - **`NON-VACUOUS`**:掃到 >0 真實/fixture artifact(非 glob-0 / 非 always-skip)。
> - **`NEGATIVE`**:注入合成違規,guard [MUST] FAIL。
> 缺任一 → D38 fail。詳 + consumer template 見 `references/guard-nonvacuous-proof.md`。
> 起點:getkm `6dbbc0a669cd` / `1df37fe37b83` / `9d864a1dd5b8`(vacuous guard = 框架最一致失敗模式)。

> 完整 guard 測試模板見 `references/GUARD-TEMPLATES.md §D38`.
...
| D38 | Non-vacuous guard proof：每個 guard test [MUST] NON-VACUOUS + NEGATIVE marker（證非 vacuous-PASS）— guard（meta，見 `references/guard-nonvacuous-proof.md`） |
| D39 | Dead-param interface-shrink：死參數 guard（tsconfig `noUnusedParameters`，`_` 前綴豁免）＋修法分流（app 死參數刪、接口契約 `_` 前綴；[NEVER] `@ts-ignore` 壓制）— guard（見 `references/GUARD-TEMPLATES.md §D39`） |
