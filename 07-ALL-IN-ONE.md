...
> 本文件是** orchestrator**：執行一次即依序跑完 03（審計/記錄）→ 02（完善程式碼與註解）→ 04（加固）→ 03（文檔收斂）→ **08（人類操作手冊，Phase E）→ 09（可重建 + 反鎖死閘門，Phase F）**。
...
- **manifest + portability-check（09，Phase F）**：`.portability.toml`（五段完整）+ portability guard 三項（secret-not-in-vars / raw-SQL / manifest 存在性）PASS + fresh-clone smoke 過（走 `09-PROJECT-PORTABILITY.md` §1.1/§5，模板 `references/PORTABILITY-GUARDS.md`）
...
## Phase F — portability gate（見 `~/Code/rules/09-PROJECT-PORTABILITY.md`）

> 本 Phase 是 07 與 09（專案可重建 + 廠商反鎖死合約）的**交接點**，作為「可發布 / 可搬移」閘門。
> [MUST] 在 Phase D 驗證全綠、Phase E（若有）完成後才跑。[NEVER] 跳過（非 conditional）。
...
任一失敗 → [NEVER] 宣告「可發布/可搬移」。方法論全在 09，[NEVER] 抄錄條文。
...
Phase E (ops-doc 收斂 → 蒸餾進 docs-hub，conditional)
   ▼
Phase F (09 portability gate → manifest/guard/smoke，可發布/可搬移閘門)
   ▼
完成報告
```
...
- [ ] **Phase F**：`.portability.toml` 五段就位、portability guard 三項（secret-not-in-vars/raw-SQL/manifest）PASS、resource ID 未明文 commit、`[vendor_lock]` 已標註、fresh-clone smoke 過
- [ ] 預算未增加
